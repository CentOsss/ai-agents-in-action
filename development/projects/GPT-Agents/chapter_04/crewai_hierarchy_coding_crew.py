...
# Убедитесь, что OPENAI_API_KEY и другие переменные (OPENAI_API_BASE, OPENAI_MODEL_NAME) загружены.
# Промпт курса указывает модель gpt-4o-mini.
...
senior_engineer_agent = Agent(
    ...
    verbose=True,
    # llm=ChatOpenAI() # Можно явно указать LLM для агента
)
...
# Задачи (такие же, как в crewai_coding_crew.py)
code_task = Task(
...
# Определение LLM для менеджера
# Использует настройки по умолчанию для ChatOpenAI (подхватит из переменных окружения OPENAI_MODEL_NAME, OPENAI_API_BASE, OPENAI_API_KEY)
# Промпт курса рекомендует gpt-4o-mini. Если он не указан в OPENAI_MODEL_NAME, 
# можно явно задать manager_llm = ChatOpenAI(model="gpt-4o-mini")
# Для модели gpt-4, как в листинге книги, было бы manager_llm = ChatOpenAI(model="gpt-4")
manager_llm = ChatOpenAI(
    # model="gpt-4o-mini", # Явное указание модели, если OPENAI_MODEL_NAME не установлен или другой
    # temperature=0 # Как в книге
)

# Команда с иерархической обработкой
crew = Crew(
...
print("\n\n######################")
print("## Starting Hierarchical Game Crew Kickoff!")
print("######################\n")
result = crew.kickoff()

print("\n\n######################")
print("## Hierarchical Game Crew Work Result:")
print("######################\n")
print(result) 